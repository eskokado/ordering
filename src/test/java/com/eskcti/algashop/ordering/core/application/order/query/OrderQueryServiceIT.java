...
import com.eskcti.algashop.ordering.core.application.order.query.ShippingData;
...
class OrderQueryServiceIT {
        ...
        public void shouldFindById() {
                ...
                Assertions.assertThat(output.getItems())
                                ...
                                .containsExactlyInAnyOrder(
                                                Assertions.tuple(order.id().toString(), "Notebook X11", 2,
                                                                new BigDecimal("3000.00")),
                                                Assertions.tuple(order.id().toString(), "4GB RAM", 1,
        ...
        public void shouldReturnEmptyPageWhenCustomerHasNoOrders() {
                Customer customer = CustomerTestDataBuilder.existingCustomer()
                                .fullName(new FullName("John", "Doe"))
                                .email(new Email("johndoe@example.com")).build();
                Customer otherCustomer = CustomerTestDataBuilder.brandNewCustomer()
                                .fullName(new FullName("Jane", "Doe"))
                                .email(new Email("janedoe@example.com")).build();
                customers.add(customer);
        ...
        public void shouldSortOrdersByPaidAtDescending() {
                ...
                orders.add(OrderTestDataBuilder.aPlacedOrder().customerId(customer.id()).build());
                orders.add(OrderTestDataBuilder.aPlacedOrder().customerId(customer.id()).status(OrderStatus.PAID)
                                .build());
                ...
                Assertions.assertThat(page.getContent().getFirst().getStatus()).isEqualTo(OrderStatus.PLACED.name());
                Assertions.assertThat(page.getContent().get(1).getStatus()).isEqualTo(OrderStatus.PAID.name());
        }
